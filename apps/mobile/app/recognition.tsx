...
export default function RecognitionScreen() {
  ...
  async function fetchRecognitions() {
    try {
      ...
      const senderIds = [
        ...new Set(
          (recData as RecognitionRow[])
            .map((r: RecognitionRow) => r.sender_id)
            .filter(Boolean)
        ),
      ];
      ...
      const merged: Recognition[] = (recData as RecognitionRow[]).map(
        (rec: RecognitionRow) => ({
          ...rec,
  ...
  const thisMonth = recognitions.filter((r: Recognition) => {
    ...
    <View className="flex-1 bg-brand-background">
      ...
      >
        ...
        {recognitions.length === 0 ? (
          ...
          recognitions.map((rec: Recognition, index: number) => {
